"""CLI entry point for all TorchGenomics commands.
...
def _cmd_gu_scan(args: argparse.Namespace) -> int:
    ...
    if args.probs:
        ...
        _probs_tmpdir = Path(tempfile.mkdtemp(prefix="torchgenomics_guscan_"))
...
def _cmd_pipeline(args: argparse.Namespace) -> int:
    ...
    if impute_method and impute_method != "mean":
        logger.warning(
            ...
            "Use `torchgenomics impute` first, then run pipeline with pre-imputed data. "
    ...
    # GxE LMM (single-trait HetLMM via pipeline) — streaming variant
        ...
        logger.warning(
            ...
            "For full control, use `torchgenomics gxe-scan --env <file>`."
        )
        model = HetLMM()
...
def _cmd_phase_poly(args: argparse.Namespace) -> int:
    ...
    if sample_ids is None or variant_ids is None:
        raise SystemExit(
            ...
            "or a raw tensor produced by 'torchgenomics dosage-call' with a sibling "
            "<prefix>.meta.json. "
...
def _load_genotype_matrix(genotype_path: str) -> torch.Tensor:
    ...
    All TorchGenomics readers in :mod:`torchgenomics.io` yield ``(G_chunk, vmeta)``
...
def _add_gu_scan_parser(subparsers: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--probs", default=None,
                   help="Path to a Phase 55 <prefix>.probs.pt from "
                        "`torchgenomics dosage-call`. When given, --genotype and "
...
def _add_common_scan_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--output", default="torchgenomics_results")
...
def _add_annotate_parser(subparsers: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--output", default="torchgenomics_annotated", help="Output file prefix")
...
def _cmd_bayes_scan_rss(args: argparse.Namespace) -> int:
    ...
    if args.ld_ref:
        ...
        # torchgenomics.io that is not yet wired. Per NA1 plan Task 11
        ...
        raise NotImplementedError(
            ...
            "reference upstream (e.g. via torchgenomics.postgwas._ld_ref_loader."
...
def _add_combine_gwas_twas_parser(subparsers: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--twas-results", required=True,
                   help="TWAS results TSV (output of `torchgenomics twas-scan` or "
