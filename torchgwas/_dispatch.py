...
Hot loops in TorchGWAS now have up to three execution paths:
...
- ``"native"`` — a hand-rolled C++ extension under ``torchgwas._native``.
...
- ``TORCHGWAS_DISABLE_NATIVE=1`` — never use the C++ path. Used by the
  test suite to exercise the Python reference and by users debugging
  numerical drift.
- ``TORCHGWAS_DISABLE_GPU=1`` — never run the dedicated GPU path. The
...
def native_disabled() -> bool:
    """Whether ``TORCHGWAS_DISABLE_NATIVE`` is set in the environment."""
    return bool(os.environ.get("TORCHGWAS_DISABLE_NATIVE"))


def gpu_disabled() -> bool:
    """Whether ``TORCHGWAS_DISABLE_GPU`` is set in the environment."""
    return bool(os.environ.get("TORCHGWAS_DISABLE_GPU"))
...
def select_path(
    ...
    *,
    ...
    has_native : bool
        Whether the relevant ``torchgwas._native._<name>_native`` extension
