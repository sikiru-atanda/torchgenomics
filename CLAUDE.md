...
Full signature: `tg.gwas(phenotype, genotype, *, covariates=None, kinship="auto", pcs="auto", trait=None, trait_type=None, models="auto", qc=True, correction="bh", output=None, device=None, verbose=True, model_options=None, env=None, regions=None, traits=None)`.

- `models=` is the caller's choice — pass an alias (`"lmm"`, `"glm"`, `"blink"`, `"farmcpu"`, `"glmm"`, `"mklmm"`, `"gxe"`, `"set"`, `"bayes"`, `"mvlmm"`, or a GAPIT-name synonym) or a list of aliases for a `GwasComparison`. `models="auto"` (the default) is an opt-in convenience, not a silent override: it prints exactly which model it picked and why (`verbose=True`, the default), and only ever picks a model that is actually wired (`lmm` for a continuous trait with kinship on, `glm` otherwise).
- **Every registry model runs through `tg.gwas` today** (`lmm`, `glm`, `blink`, `farmcpu`, `glmm`, `mklmm`, `gxe`, `set`, `bayes`, `mvlmm`). `models="gxe"` requires `env=` (a `pandas.Series` indexed by sample id, a 1-D array in sample order, or a path to an env TSV) and raises a friendly `ValueError` if omitted. `models="set"` requires `regions=` (a path to a chrom/start/end[/id] regions file, or a `pandas.DataFrame` with those columns) and likewise raises `ValueError` if omitted. `models="mvlmm"` (multi-trait GWAS) requires `traits=[...]` naming >=2 columns of a multi-column phenotype (a `pandas.DataFrame` or a path to one) and raises a friendly `ValueError` naming both "mvlmm" and "traits" if omitted or under 2; it returns a joint multi-trait result whose association table carries `P_JOINT` instead of a single-trait `P` column. `models="bayes"` is a fine-mapping run, not a p-value scan — its `GwasResult.top_hits` carries `PIP` (posterior inclusion probability) and credible-set membership instead of a `P` column, and `lambda_gc` is `None`. Pass per-model settings via `model_options={"glmm": {"family": "ordinal"}}` (also available as CLI `gwas --model-options '<json>'`).
