...
def make_synth(n: int = 200, m: int = 50, K: int = 2, seed: int = 0) -> dict:
    ...
    fst = 0.1
    base = 0.1 + 0.8 * torch.rand(m, dtype=dtype)
    a = base * (1 - fst) / fst
    b = (1 - base) * (1 - fst) / fst
    ...
    all_ancestry_idx = torch.multinomial(adm, num_samples=2, replacement=True)  # (n, 2)
    ...
    reff = L @ torch.randn(n, dtype=dtype)
    X0 = torch.stack([torch.ones(n, dtype=dtype),
                      torch.randn(n, dtype=dtype)], dim=1)  # intercept + 1 cov
    y_cont = 1.0 + 0.3 * X0[:, 1] + signal + reff + torch.randn(n, dtype=dtype)
    prob = torch.sigmoid(y_cont - y_cont.mean())
    y_bin = torch.bernoulli(prob)
