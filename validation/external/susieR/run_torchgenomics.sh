...
START=$(date +%s.%N)
/usr/bin/time -v python3 -m torchgenomics bayes-scan-rss \
...
with open("${OUT_DIR}/torchgenomics_elbo.txt", "w") as f:
    f.write(f"{result.elbo}\n")
# Convergence flag (replaces the absolute-ELBO parity metric).
with open("${OUT_DIR}/torchgenomics_converged.txt", "w") as f:
    f.write("True" if result.converged else "False")
print(f"torchgenomics ELBO = {result.elbo:.6g} (converged={result.converged}, n_iter={result.n_iter})")
PYEOF
