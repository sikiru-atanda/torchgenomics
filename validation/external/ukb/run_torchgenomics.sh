#!/usr/bin/env bash
# Run TorchGenomics LMM scan + LDSC h² estimate on the UKB chr22 fixture.
#
# Streaming-first invariants (per `feedback_streaming` memory):
#   - lmm-scan reads chunks via `torchgenomics.io.PlinkBedReader.iter_chunks` —
...
#
# Outputs:
#   torchgenomics_outputs/lmm_chr22.assoc.tsv
#   torchgenomics_outputs/lmm_chr22.time.log    (peak RSS + wall time)
#   torchgenomics_outputs/h2_chr22.json         (h² point estimate + SE + extras)
...
# --- 1. TorchGenomics LMM scan (streaming) --------------------------------------
...
echo "[ukb tg-run] (1/2) TorchGenomics lmm-scan on ${GENO_ARG}"
...
# --- 2. TorchGenomics LDSC h² estimate ------------------------------------------
# `torchgenomics` ships `compute_ld_scores` and `ldsc_h2`. There isn't currently a
...
#   3. Calls `torchgenomics.postgwas.ldsc_h2(chi2, ld_scores, w_ld, n, m_total)`.
...
echo ""
echo "[ukb tg-run] (2/2) TorchGenomics h² (chi² + LD-score regression on chr22)"
...
# Parent torchgenomics import path:
sys.path.insert(0, str(Path(__file__).resolve().parents[4]))
from torchgenomics.postgwas import ldsc_h2  # noqa: E402
...
/usr/bin/time -v -o "${H2_TIME_LOG}" \
    python3 "${TG_PY_DRIVER}" \
        ...
        --out-json "${H2_OUT}" \
    || {
        echo "[ukb tg-run] FAIL: TorchGenomics h² driver exited non-zero"
