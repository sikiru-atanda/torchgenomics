#!/usr/bin/env python3
"""run_torchgwas.py -- fit RandomRegressionLMM on the same fixture.

Reads validation/specialty/rr/data/{long_pheno.tsv,geno.tsv,truth.json}
and writes validation/specialty/rr/outputs/{torchgwas_null.tsv,
torchgwas_causal.tsv} using the same name -> value schema as
...
from torchgwas.models.rr_lmm import RandomRegressionLMM  # noqa: E402
...
def main() -> int:
    ...
    (out_dir / "torchgwas_null.tsv").write_text(
        "name\tvalue\n" + "\n".join(null_rows) + "\n"
    )
    print("[tg] wrote torchgwas_null.tsv", flush=True)
    ...
    (out_dir / "torchgwas_causal.tsv").write_text(
        "name\tvalue\n" + "\n".join(causal_rows) + "\n"
    )
    print("[tg] wrote torchgwas_causal.tsv", flush=True)
    return 0
