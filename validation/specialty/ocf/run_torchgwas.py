# -*- coding: utf-8 -*-
# OCFLMM TorchGWAS harness for the C6 fixture.
#
...
#     TorchGWAS does this in score_chunk via project_genotype=True.
...
from torchgwas.models.ocf_lmm import OCFLMM     # noqa: E402
...
def main():
    ...
    print(f"[torchgwas] reps={reps} n={n} m={m_snps} causal=idx{causal_idx0} K={args.k} theta0={theta0:.4f}")
    ...
    df.to_csv(out_dir / "torchgwas_results.tsv", sep="\t", index=False)
    ...
    with open(out_dir / "torchgwas_summary.json", "w") as f:
        json.dump(summary, f, indent=2)

    print("[torchgwas] DONE. empirical_coverage={0:.3f} mean_theta_hat={1:.4f} mean_bias={2:.4f} mean_se={3:.4f}".format(emp_cov, float(df["theta_hat"].mean()), mean_bias, float(df["se"].mean())))
