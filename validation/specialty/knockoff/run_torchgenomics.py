"""Run TorchGenomics KnockoffLMM across all replicate fixtures.
...
from torchgenomics.models.knockoff_lmm import KnockoffLMM  # noqa: E402
...
def main():
    ...
    per_rep_rows = []
    print(f"[torchgenomics] running {n_replicates} replicates at target_fdr={args.target_fdr}")
    ...
    tsv_path = out_dir / "torchgenomics_per_replicate.tsv"
    ...
    (out_dir / "torchgenomics_summary.json").write_text(json.dumps(summary, indent=2))
    print(
        "[torchgenomics] mean_fdr={:.4f} +/- {:.4f}   mean_power={:.4f} +/- {:.4f}".format(
