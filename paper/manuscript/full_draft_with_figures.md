...
The streaming contract is necessary but not sufficient: per-chunk work also has to be fast. TorchGWAS ships **31 pybind11 C++ extensions** under `csrc/`, OpenMP-parallelised where it pays off. The extensions are `optional=True` in `setup.py`, so `pip install` succeeds without a C++ toolchain; the device-aware dispatcher routes each call to native if available, GPU if a dedicated CUDA kernel exists, else the pure-PyTorch reference. The Python body is preserved verbatim as the algorithmic specification, so `TORCHGWAS_DISABLE_NATIVE=1` falls through cleanly. The realistic-size benchmark spans **1.0×–20,183× with a median of ~100×**: top kernels include PCHT compat-pair (20,183×), Gabriel LD blocks (9,211×), mediation σ_u/σ_v (3,982×), TWAS quantile-norm (2,164×), KNN imputation (645×), LDpred2 Gibbs (352×). A complementary Python-side change caches the Cholesky factor of R across SuSiE-RSS IBSS iterations so each ELBO solve becomes O(p²) substitution — at p=2,000 the end-to-end fit_rss drops from 5.8s to 0.53s (11×).
