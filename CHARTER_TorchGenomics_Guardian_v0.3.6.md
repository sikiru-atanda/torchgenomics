# CHARTER_TorchGenomics_Guardian.md
**Project:** TorchGenomics  
**AI Agent Codename:** *TorchGenomics Guardian*  
**Version:** v0.3.5  
**Date:** 2026-04-03  
**Owner:** TorchGenomics Core Team (Computational Biology / Quantitative Genetics / HPC)
...
- **Format-aware imputation policy:** internal inference-safe imputation available for all formats; **external haplotype-aware imputation (e.g., Beagle) requires VCF/BCF** (TorchGenomics may convert HapMap/TXT → VCF when possible; otherwise warns/errors).
...
- **Imputation Decision Matrix by Format**:
  ...
  - TorchGenomics conversion rules (HapMap/TXT/PLINK → VCF) with explicit prerequisites and failure modes.
...
## 1. Mission
Build and maintain a modular, GPU-accelerated GWAS library (**TorchGenomics**) that:
...
These are the baseline engines TorchGenomics must run with high fidelity and calibration.
...
- **Definition of `loco="auto"`:** if K is derived from genome-wide variants and chromosome labels are available, TorchGenomics builds a per-chromosome `K_chr` by excluding **all** variants on the tested chromosome. If K is user-supplied (not variant-derived) or chromosome labels are unavailable, LOCO is disabled and this is recorded in metadata.
...
These features are **not optional add-ons**; they are routine novelty shipped with TorchGenomics, even for established models.
...
TorchGenomics must normalize *all* supported genotype inputs into a single internal representation so that QC, imputation, GRM construction, and association testing are consistent across formats.
...
- **Dosage-aware constraint (DS/GP):** if a multiallelic site carries DS/GP (or any fractional dosage field), TorchGenomics must **drop the site by default** unless the user enables `--allow_multiallelic_split_ds` **and** a validated dosage splitter is configured. Rationale: naïve split can silently corrupt dosages and invalidate inference.

**5.0.5 Polyploid / fractional dosage sanity checks**
- For any analysis run, TorchGenomics must validate:
...
- If input format cannot represent polyploid dosage correctly (e.g., diploid-only allele strings), TorchGenomics must warn and require an alternative format (VCF/BCF DS/GP or dosage matrix).
...
TorchGenomics must implement and/or record:
...
**Polyploid note:** PLINK1 `.bed` is treated as *diploid-first*. Polyploid workflows should prefer VCF/BCF with DS/GP or dosage matrices. If PLINK1 is used for polyploid, TorchGenomics requires explicit user confirmation and strict validation.
TorchGenomics must accept:
...
TorchGenomics accepts HapMap genotype files (e.g., `.hmp`, `.hmp.txt`, `.hmp.gz`) **only when the following schema requirements are met**.
...
- Conversion requires unambiguous biallelic alleles and valid coordinates (`chrom`, `pos`). If REF/ALT cannot be harmonized (e.g., strand ambiguity without a reference), TorchGenomics must refuse external imputation and fall back to Tier A inference-safe imputation or error based on user policy.

---

### 5.5.2 Delimited text genotype matrices (TXT/TSV/CSV) contract
TorchGenomics accepts `.txt/.tsv/.csv` genotype matrices **only with an explicit schema** so that downstream QC, allele alignment, and optional VCF conversion are well-defined.
...
**Ploidy**
- Default ploidy is `P=2`. If `P != 2`, the user must specify ploidy explicitly and TorchGenomics will validate dosage ranges and/or GT token counts accordingly.
...
- **External imputation (e.g., Beagle) requires VCF/BCF** and therefore requires that TorchGenomics can emit a valid VCF:
...
- If these prerequisites are not met, TorchGenomics must not attempt external imputation and will fall back to Tier A inference-safe imputation or error depending on user policy.
...
TorchGenomics must enforce the following:
...
TorchGenomics supports **imputation policies**, but must clearly separate their intended use:
...
- Haplotype-aware imputation tools (e.g., Beagle) are supported only via an adapter interface and **require VCF/BCF** inputs.
  - If user provides HapMap/TXT, TorchGenomics may **convert to VCF** (biallelic normalization required) before calling external imputation.
  - If conversion is not possible (e.g., ambiguous alleles, missing required metadata), TorchGenomics must warn and fall back to Tier A or error based on configuration.
...
> Important constraint: external imputation tools may not accept certain formats directly (e.g., “TXT” matrices). TorchGenomics must not claim otherwise; it must convert to a supported format (typically VCF/BCF) or skip external imputation.

#### 5.6.1 Imputation Decision Matrix by Input Format (Required)
TorchGenomics must choose an imputation path that is both *compatible* with the file format and *appropriate* for inference.
...
**Conversion rules**
- TorchGenomics may convert PLINK/HapMap/TXT → VCF for external imputation only if:
...
- If prerequisites fail, TorchGenomics must:
...
- Any procedure that performs selection (multi-locus, model-family screening) must use:
  ...
  otherwise TorchGenomics must not claim calibrated p-values for post-selection statistics.

### 6.1.1 Selection-aware inference labeling (required)
Some methods select variables during modeling; TorchGenomics must label outputs accordingly:
...
3) Pipelines:
   - TorchGenomicsConfig unified config object
   - one-call workflows (format-agnostic): `TorchGenomics.run_gwas(geno=..., ...)`
4) Reports:
...
TorchGenomics must include a reproducible benchmarking suite that compares:
- established baselines (association + multi-locus + scalable biobank LMMs)
- TorchGenomics baselines (GLM/LMM/mvLMM/FarmCPU/BLINK)
...
To prevent ad hoc benchmarking, TorchGenomics must ship explicit wrappers for external baselines under:
...
- Each runner must:
  ...
  - emit a standardized **Parquet** results file with the TorchGenomics benchmark schema:
