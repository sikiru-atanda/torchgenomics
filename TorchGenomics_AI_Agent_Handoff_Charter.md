# TorchGenomics — AI Agent Handoff Charter and Stepwise Build Blueprint
...
TorchGenomics must be a statistically rigorous, computationally efficient GWAS library for plant and animal
...
### Current tool limitations that TorchGenomics addresses

| Tool | Key limitation TorchGenomics overcomes |
|---|---|
...
2. **GPU-accelerated multi-trait mvLMM**: To our knowledge, no existing tool provides GPU-batched Kronecker-structured mvLMM scanning. TorchGenomics implements the Efficient Eigen-Decomposition (EED) trick with batched `(n, d, d)` Cholesky solves on GPU, reducing per-SNP complexity from O(n^3 d^3) to O(n d^3) with GPU-parallel execution across SNP chunks.

3. **GPU-accelerated multi-locus methods (FarmCPU/BLINK)**: These iterative methods have linear time complexity in n (the most scalable GWAS models), but to our knowledge, no existing implementation exploits GPU. TorchGenomics batches the FEM/REM iterations and BIC selection on GPU tensors.
...
7. **Interpretation-safety layers (routine novelty)**: Every TorchGenomics run produces a covariate audit (detecting heritable covariates / colliders; Aschard et al. 2015 AJHG), LD-block attribution, and per-SNP causal-safety diagnostics (LD proxy risk, confounding sensitivity, within-family attenuation when available). To our knowledge, no existing GWAS tool ships these diagnostics as default output.
...
- **Imputation/phasing orchestration layer**: TorchGenomics provides an integrated orchestration layer for imputation and phasing, with native simple imputers and wrapper-based support for best-in-class external tools (BEAGLE, IMPUTE5, Minimac4, STITCH). TorchGenomics does not reimplement these algorithms; it manages format conversion, invocation, quality tracking, and pipeline chaining. (Phase 2 for wrappers; Phase 20 for novel GPU imputation research.)
...
TorchGenomics must faithfully replicate GEMMA's mathematical machinery before extending it. This section
...
- TorchGenomics must match this: AI-REML or Newton-Raphson for the 2-parameter (sig_g, sig_e) case.
...
- GEMMA uses a Newton-like approach; TorchGenomics provides two paths:
...
5. **Eigenvalue precision**: Using FP32 for eigendecomposition loses ~7 decimal digits. GEMMA uses FP64. TorchGenomics must too.
...
Methods that perform variable selection during modeling produce post-selection statistics. TorchGenomics must label outputs accordingly and not claim calibrated marginal p-values unless debiasing is applied.
...
- **LOCO (Leave-One-Chromosome-Out)**: Default `loco="auto"` for univariate LMM GWAS when K is built from genome-wide SNPs. If K is derived from genome-wide variants and chromosome labels are available, TorchGenomics builds per-chromosome K by excluding all variants on the tested chromosome. If K is user-supplied or chromosome labels are unavailable, LOCO is disabled and recorded in metadata.
...
### 5b-v. TorchGenomics GPU implementation strategy
...
5. **PyTorch autograd for variance components**: Unlike the Fortran BLUPF90 implementation which assumes known variance components, TorchGenomics can use autograd to differentiate through the NR iterations for joint estimation of variance components and thresholds — a potential novel extension.
...
- **Post-V1 implementation, in-scope for platform**: Binary and ordinal traits are part of TorchGenomics's platform vision, scientifically anchored in Bermann et al. (2026). Implementation is deferred to Phase 16, after V1 Gaussian-trait models are validated.
- **Prerequisites**: Requires V1 LMM infrastructure (GRM, eigendecomposition, MME solvers, PCG).
- **Module**: `torchgenomics.models.ThresholdLinearModel` implementing `BaseModel` protocol with `fit_null()` solving the NR/EM iteration and `score_chunk()` computing ssGWAS p-values.
...
encoding, missing required columns). TorchGenomics must handle this gracefully.
...
The HapMap format is widely used in plant genetics (GAPIT, TASSEL, rMVP). TorchGenomics must accept
...
**TorchGenomics HapMap parsing rules:**
...
5. **Polyploid HapMap**: Some polyploid pipelines encode dosage directly as integers (`0`, `1`, `2`, `3`, `4` for tetraploid) in place of letter genotypes. TorchGenomics must detect this automatically (all genotype values are single digits) and parse accordingly.
...
arrays, or dosage called from sequencing), TorchGenomics accepts plain-text numeric files.
...
**TorchGenomics CSV/TSV/TXT parsing rules:**
...
Sample alignment is a critical production edge — mismatches between .fam/.psam IDs and phenotype/covariate IDs silently produce wrong results in other tools. TorchGenomics must enforce alignment deterministically:
...
TorchGenomics must implement a `DataLoader` that:
...
4. **Reports a pre-flight summary** before any GWAS run:
   ```
   TorchGenomics Pre-flight Check:
...
torchgenomics convert --input data.hmp.txt --output data --format bed

# Convert numeric CSV (with separate map) to PLINK BED
torchgenomics convert --input genotype.csv --map marker_map.txt --output data --format bed

# Convert BGEN to Zarr (for out-of-core processing)
torchgenomics convert --input data.bgen --sample data.sample --output data.zarr --format zarr

# Validate a dataset without running GWAS (works with any format)
torchgenomics validate --genotype data.hmp.txt --phenotype pheno.txt --report report.json
torchgenomics validate --genotype dosage.csv --map snp_info.txt --phenotype pheno.csv --report report.json
```

### 6i. Variant QC filters and allele-frequency Parquet output

TorchGenomics must compute per-variant QC statistics during preprocessing and persist them to a Parquet file. This serves three purposes: (1) reproducible QC — the exact filter decisions are auditable; (2) QA against GAPIT/GEMMA defaults — users can verify that TorchGenomics computes identical allele frequencies and filter counts; (3) downstream analysis — allele-frequency columns are required for meta-analysis, LD score regression, and polyploid gene-action model selection.
...
After computing all statistics and applying filters, TorchGenomics writes a Parquet file (`{prefix}_variant_qc.parquet`) containing all columns above plus a `FILTER_PASS` boolean column. This file is written **before** the GWAS scan begins and is referenced in the results output.
...
Before running GWAS, TorchGenomics can run a lightweight covariate audit (enabled by default as warn-only):
...
For each SNP and/or locus, TorchGenomics produces diagnostic components:
...
IMPUTE5, Minimac4) separately before GWAS — a manual, error-prone step. TorchGenomics integrates
imputation as an optional pre-GWAS pipeline stage.

### 7b. Imputation tiers (inference safety contract)

TorchGenomics classifies imputation methods by their suitability for downstream inference:
...
TorchGenomics does not reimplement BEAGLE's Li-and-Stephens HMM from scratch (that would be a multi-year
...
2. **External tool wrappers** (pluggable, user selects backend):
   ...
   TorchGenomics wraps these tools with:
   - Automatic format conversion (TorchGenomics internal format -> tool's required input -> parse output back).
   - Unified interface: `torchgenomics impute --method beagle|impute5|minimac4|stitch --genotype data.vcf`
...
External haplotype-aware imputation requires VCF/BCF. TorchGenomics must enforce:
...
3. **Novel GPU-accelerated imputation** (Phase 19, **DONE**):
   ...
   - CLI: `torchgenomics impute --method li-stephens|deep-learning --ploidy K`
...
useful for haplotype-based association tests. TorchGenomics provides:

- **Wrapper for BEAGLE phasing**: BEAGLE 5.5's two-stage phasing algorithm handles both common
  and rare variants efficiently. TorchGenomics calls BEAGLE, parses phased output, and stores
...
After imputation, TorchGenomics automatically:
...
torchgenomics impute --genotype data.bed --method mean --output data_imputed.bed
...
torchgenomics pipeline --genotype data.vcf.gz \
...
methods improve power without inflating false positives. TorchGenomics must provide a complete toolkit.
...
permutations x 500,000 SNPs = 5 billion test statistics. TorchGenomics makes this practical:
...
TorchGenomics aims to be, to our knowledge, the first GPU-accelerated, multi-model polyploid GWAS tool.

### 9a-2. PloidyConfig (user-facing, enforceable)

TorchGenomics exposes a public configuration object that determines how ploidy is interpreted and validated.
...
**Polyploid format constraints**: PLINK1 `.bed` is diploid-first; polyploid workflows should prefer VCF/BCF with DS/GP or dosage matrices. If PLINK1 is used for polyploid, TorchGenomics requires explicit user confirmation and strict validation.
...
TorchGenomics stores genotypes as float tensors with values in [0, k]. This representation:
...
introduced these models; TorchGenomics generalizes them to arbitrary ploidy k:
...
For each SNP, TorchGenomics can test all gene-action models and report the best-fitting model
alongside the standard additive result. This is critical because the true gene action is unknown.

**Diplo-general model**: GWASpoly implements a "diplo-general" model that first diploidizes genotypes ({0->0, 1..k-1->1, k->2}) then encodes as a factor (one-hot). TorchGenomics should implement this as an additional multi-column model alongside the existing "general" model. Note: TorchGenomics's `diplo-additive` uses `min(d, k-d)` which provides finer allelic resolution than GWASpoly's diploidized encoding — both are valid choices for different biological hypotheses.

### 9d. Polyploid GRM / kinship matrix

The kinship matrix K must account for ploidy. TorchGenomics implements multiple GRM methods, benchmarked against AGHmatrix (Amadeu et al. 2016) and GWASpoly:
...
- **GWASpoly kinship**: Already benchmarked — TorchGenomics VanRaden matches perfectly (r=1.000, Section 13b).

### 9e. Multi-trait polyploid GWAS (novel)

To our knowledge, no existing tool supports multi-trait GWAS for polyploids. TorchGenomics extends the mvLMM (Section 4b)
...
- **Dosage calling quality**: For polyploids, dosage calling is less certain than for diploids.
  TorchGenomics accepts probability vectors P(d=0), P(d=1), ..., P(d=k) per sample-SNP and
...
GWASpoly implements a structured post-GWAS workflow for polyploids that TorchGenomics should replicate:
...
- **P3D=TRUE**: Estimate variance components (σ²_g, σ²_e) once from the null model, then scan all markers with fixed variance components. This is the standard approach and matches TorchGenomics's current behavior.
...
TorchGenomics should support both via a `p3d: bool = True` parameter on the scan method.

#### Marker effect estimates in scan output

GWASpoly reports marker effect estimates (betas) alongside p-values. TorchGenomics's `ScanResult` already includes `beta` and `se` fields for Wald and LRT tests, but these are NaN for score tests. Ensure:
...
TorchGenomics should support two M.eff algorithms for polyploid multiple-testing correction:
...
**GAPIT3 polyploid note**: GAPIT3 can accept polyploid dosage as numeric input, but it does NOT have ploidy-aware GRM normalization (uses diploid `2p(1-p)` regardless), no gene-action models, and no polyploid-specific QC. It is useful only as an additive-model baseline. TorchGenomics should match GAPIT3's additive p-values when using the same (diploid-style) GRM normalization.
...
- **ASReml** uses AI-REML as primary with PX-EM warm start — the gold standard for breeding models. TorchGenomics adopts this pattern.
- **SAIGE** uses AI-REML with PCG — TorchGenomics includes this as Mode E for large-scale.
- **TorchGenomics adds Mode C** (LBFGS-autograd) which no other tool has: it lets users define new log-likelihoods and get optimization "for free" via autograd.
- **TorchGenomics adds Mode G** (TRIAD-REML) which goes further: exact Hessian-vector products via autograd double-backward, combined with trust-region globalization. No other GWAS tool uses exact second-order curvature — GEMMA, GCTA, and ASReml all rely on the Average Information approximation, which is inaccurate far from the optimum.
...
| **36** | Post-GWAS (LDSC, Meta-analysis, Clumping) | `torchgenomics.postgwas` — LDSC h²/rg, meta-analysis (IVW/DL/Stouffer/RE2), LD clumping | Essential downstream analysis | **DONE**: 30 tests pass |
| **40** | Polygenic Score (PGS) Construction | `torchgenomics.pgs` — C+T, LDpred2 (Inf/Grid/Auto), PRS-CS; allele harmonization; `score_individuals`; MCMC diagnostics; `pgs-fit` + `pgs-score` CLI | Tier 1 + Tier 2 PGS methods | **DONE**: 75 tests pass |
| **41a-41z** | Native C++ Accelerators | 26 pybind11 C++ extensions for hot loops (PRS-CS, LDpred2, C+T, PELT, ESS, Gabriel, Big-LD, DP-optimize, CC-graph, GWAS-aligned, Spine, LD-decay-signal, greedy-MWIS, uncertainty-blocks, Wall-Pritchard, impute-mode/knn/ld, CAVI, SPA, LDSC-jackknife, HWE/HWE-DR, cross-pop, postgwas-clump); OpenMP parallelization for 6 kernels; dispatch helper `select_path`; GPU imputation kernels; 12/24 kernels show >50× speedup | Performance acceleration with Python fallback | **DONE**: ~250 native tests pass; all toggleable via `TORCHGENOMICS_DISABLE_NATIVE=1` |
| **42** | S-LDSC + Colocalization | `torchgenomics.postgwas._sldsc` (partitioned LDSC, Finucane 2015) + `_hyprcoloc` (multi-trait coloc, Foley 2021) + classical `coloc_pairwise` (Giambartolomei 2014) | Functional enrichment + causal variant sharing | **DONE**: 17 tests pass |
| **43** | Visualization + PVE | `torchgenomics.viz` — Manhattan, Circos Manhattan, Miami, QQ, Haploview LD triangle; `torchgenomics.stats.pve` — marginal + joint PVE | Pub-quality plots + variance explained | **DONE**: 37 tests pass |
...
- All output artifacts must include: artifact schema version, TorchGenomics version, dependency versions (torch, numpy, IO backends), device info, dtype policy.
- Breaking artifact schema changes require MAJOR version bump.

## 17. Novel Scientific Contributions (Beyond V1 Replication)

These models represent TorchGenomics's contribution to the broader genetics community — not just faster
...
**Why TorchGenomics can do this**: The multi-kernel V cannot be diagonalized by a single eigendecomposition,
which is why GEMMA cannot support it. But TorchGenomics's LBFGS-autograd optimizer (Mode C) doesn't need
...
mixed-precision kernel ridge regression for epistasis on the Frontier supercomputer — TorchGenomics
...
**TorchGenomics model** (multi-environment with Kronecker structure):
```
Y (n x d) = X @ B + G + GxE + E
...
**Why TorchGenomics can do this**: The Kronecker-structured GxE model extends the mvLMM framework
...
**TorchGenomics implementation** (three complementary model families):
...
**CLI**: `torchgenomics met-scan --parameterization reaction_norm|per_env --vg-structure "fa(k)" --kernel-files dom.npy`
...
**TorchGenomics implementation** (two inference methods):
...
**CLI**: `torchgenomics bayes-scan --method susie|cavi --n-signals 10 ...`
...
**TorchGenomics opportunity**: Bring the core algorithmic ideas (INT8/FP16 tensor cores for distance
matrices, mixed-precision Cholesky solver) to commodity single-GPU hardware. TorchGenomics's AMP
...
prerequisite. This positions TorchGenomics at the intersection of HPC genomics and accessible tooling.
...
**TorchGenomics contribution**: To our knowledge, the first GPU-accelerated implementation of multi-trait threshold-linear GWAS:
...
- **Autograd-based threshold and variance component estimation**: Unlike BLUPF90 which assumes known variance components and thresholds (estimated via prior Gibbs sampling), TorchGenomics can leverage PyTorch autograd to differentiate through the NR iterations for joint estimation — a potential methodological advance.
- **Polyploid categorical-trait GWAS**: Combined with TorchGenomics's ploidy-aware genotype encoding (Section 9), this enables threshold-linear GWAS for polyploid organisms — to our knowledge, entirely novel. Disease resistance scoring in tetraploid potato, lodging scores in hexaploid wheat, and survival traits in polyploid aquaculture species become accessible.
...
**Scientific context**: This is NOT a GWAS model but a causal inference module that uses GWAS results and the TorchGenomics infrastructure. It estimates causal effects of modifiable exposures T on outcomes Y using Double/Debiased ML (Chernozhukov et al. 2018), distinct from OCF-LMM (which tests SNP effects with debiased nuisance estimation).
...
**Implementation**: `torchgenomics/optim/triad_reml.py`. Selected via `NumericalConfig(multi_trait_reml_method="triad")`. Falls back to LBFGS on failure.
...
**TorchGenomics implementation**:

1. **Bases**: `torchgenomics/linalg/basis.py` ships Legendre polynomials, B-splines, and 2D tensor-product P-splines, plus an `evaluate_basis_at` helper for predicting at arbitrary times.
2. **`RandomRegressionLMM`** (`torchgenomics/models/rr_lmm.py`): Two modes —
...
3. **K_coef structures** (`torchgenomics/optim/rr_reml.py`): `unstructured`, `diagonal` (per-channel EMMA), and `fa(k)` (`Vg = Λ Λᵀ + diag(ψ)`).
...
6. **`SpatioTemporalRR`** (`torchgenomics/models/rr_spatial.py`): Two-step deconfounding wrapper — penalized 2D P-spline fit on (row, col) field coordinates removes a fixed-effect spatial surface from `Y_long` *before* the temporal RR-LMM. Random-effect smoother path is deferred.
7. **CLI**: `torchgenomics rr-scan` (29th overall subcommand). Outputs `.rr.tsv` (per-SNP all four base tests), `.rr_at_t.tsv` (per-SNP × time when `--eval-times` given), `.rr_null.json`.
...
**TorchGenomics implementation** — `RandomRegressionMultiEnvLMM` (`torchgenomics/models/rr_met.py`, ~918 LOC), thin composition over Phase 25 `MultiTraitMultiEnvLMM`:
...
8. **CLI**: `torchgenomics rr-met-scan` (30th overall subcommand).
...
|---|---|
| Truth ↔ TorchGenomics `fa(2)` | **0.241** |
| Truth ↔ BLUPF90 `XFA 2`   | 0.599 |
| Torch ↔ BLUPF90           | 0.638 |

TorchGenomics `fa(2)` is ~2.5× closer to truth than BLUPF90 XFA on the same data. Both methods fit a rank-2 FA approximation; neither can hit zero against separable Kron truth, but TorchGenomics gets dramatically closer. Harness: `benchmark/benchmark_rr_blupf90.py --mode met` and `--mode met-fa --fa-envs E --fa-rank k`. The harness gracefully falls back to a BLUPF90 unstructured fit if a future build rejects the XFA option.
...
**TorchGenomics model**: `GULM` in `torchgenomics/models/gu_lmm.py`. Implements a dosage-variance–corrected score test where the variance of the test statistic accounts for imputation uncertainty via `diag(V_dosage)`. Reuses the LMM null fit and P-operator infrastructure.
...
**TorchGenomics model**: `LROLMM` in `torchgenomics/models/lro_lmm.py`. Per-block kinship subtraction — for each LD block, subtracts that block's contribution from the full GRM and re-fits the null. Finer-grained proximal decontamination than per-chromosome LOCO.
...
**TorchGenomics implementation**: `torchgenomics/stats/ihw.py` and `torchgenomics/stats/adapt.py`. IHW bins p-values by covariate strata and optimizes per-stratum weights. AdaPT uses EM to learn a covariate-dependent rejection threshold. Both integrated into the `--correction` CLI flag.
...
**TorchGenomics model**: `SurvivalGLMM` in `torchgenomics/models/survival_glmm.py`. Cox PH frailty model with Breslow-Clayton PQL for null fitting, martingale residual score test for scanning, and SPACox-style SPA for extreme p-values. Polyploid support included.

**Tests**: 25 tests in `tests/test_survival_glmm.py`.

### 12u. Post-GWAS Infrastructure (Phases 36-37, 42, 44-45) — **DONE**

**TorchGenomics implementation** in `torchgenomics/postgwas/`:
...
**TorchGenomics implementation** in `torchgenomics/pgs/`:
...
**TorchGenomics implementation**: 26 pybind11 C++ extension modules in `csrc/` and `torchgenomics/_native/`, each providing a drop-in replacement for a specific Python hot loop:
...
- Central dispatch via `select_path()` in `torchgenomics/_dispatch.py`.
...
**TorchGenomics implementation** in `torchgenomics/viz/`:
...
`torchgenomics/stats/pve.py`: per-marker PVE (marginal GAPIT-compatible + joint semi-partial R²).

**Tests**: 37 tests in `tests/test_viz.py` + `tests/test_pve.py`.

### 12y. Haplotype-Based GWAS (Phases 46-47) — **DONE**

**Scientific gap**: Single-SNP GWAS cannot capture multi-allelic haplotype effects where combinations of alleles on a chromosome segment influence the trait. TorchGenomics had 13 LD block detection methods but no haplotype association testing. Motivated by livestock genetics showing haplotype GWAS detecting signals invisible to single-SNP analysis.

**TorchGenomics implementation**:
- **Phase 46** (`torchgenomics/models/haplotype_gwas.py`): Four methods — HTR (Zaykin et al. 2002) F-test/LRT, block-based scan (uses any of 13 `detect_blocks()` methods), moving-window scan, haplotype-SKAT (Davies mixture chi-squared). Multi-locus EM for haplotype inference from unphased genotypes (extends Hill 1974 EM). LMM correction via rotated eigenspace WLS and P-operator.
- **Phase 46 novel** (`torchgenomics/models/haplotype_novel.py`): PCHT (phase uncertainty correction), HHCT (hierarchical collapsing with Meinshausen 2008 FWER), HSKAT (exponential-decay Hamming kernel SKAT), HapGxE (haplotype × environment interaction F-tests), BayesHap (SuSiE IBSS on haplotype dosages).
- **Phase 47** (`torchgenomics/models/haplotype_multi.py`): Three thin-composition classes extending haplotype testing to multi-dimensional phenotypes: `HaplotypeMultiEnvGWAS` (→ MultiEnvLMM), `HaplotypeMultiTraitGWAS` (→ MultiTraitLMM), `HaplotypeMTMETGWAS` (→ MultiTraitMultiEnvLMM). Core `_haplotype_gls_wald()` builds `(c*p + h*p) x (c*p + h*p)` GLS per block; contrast algebra via `_wald_subblock()` and `_wald_contrast()`.
...
CLI usage: `torchgenomics lmm-scan --genotype data.zarr --phenotype pheno.txt --approx-method sparse --sparse-threshold 0.05`
...
### 13b. Phase 13 Polyploid Benchmark — TorchGenomics vs GWASpoly Reference
...
| GWASpoly Model | Encoding | TorchGenomics Equivalent |
|----------------|----------|---------------------|
| additive | dosage as-is (0..k) | `additive` |
| j-dom-alt | 1 if dose >= j | `j-dom` |
| j-dom-ref | 1 if dose > k-j | `(k-j+1)-dom` (approximate) |
| diplo-additive | {0->0, 1..k-1->1, k->2} (diploidized) | **differs**: TorchGenomics uses min(d, k-d) |
...
For tetraploid: `1-dom-alt` = TorchGenomics `1-dom`, `2-dom-alt` = TorchGenomics `2-dom`, `2-dom-ref` = TorchGenomics `3-dom`.

#### Multi-Environment Model

GWASpoly uses all 1249 observations with a Z incidence matrix mapping observations to 957 genotypes: V = sig2_g * Z @ K @ Z^T + sig2_e * I. TorchGenomics replicates this by expanding K to observation level (Z @ K_geno @ Z^T) and including env factor dummies in X0.

#### Benchmark Results (P3D mode — matching GWASpoly)

| TorchGenomics Model | GWASpoly Ref | n_common | r(-log10p) | Top-50% Overlap |
|----------------|-------------|----------|-----------|-----------------|
...
**Result**: All gene-action models with matching encoding achieve r > 0.999 and 100% top-50 overlap with GWASpoly. The diplo-additive difference is expected — GWASpoly uses a "diploidized" encoding ({0->0, 1..3->1, 4->2}) while TorchGenomics uses min(d, k-d), which is a deliberate design choice supporting finer allelic resolution.
...
VanRaden GRM computed by TorchGenomics (`grm_vanraden` with ploidy=4) shows:
...
### 13c. GRM Benchmark — TorchGenomics vs AGHmatrix (Polyploid Kinship)
...
| Simple GLM may not beat PLINK 2.0 on CPU | Any n | PLINK 2.0 uses hand-tuned SIMD intrinsics for GLM. TorchGenomics GLM should target GPU advantage; do not claim CPU GLM parity with PLINK 2.0. |
...
```
torchgenomics/                      # installable package
  __init__.py
  config.py
...
TorchGenomics_AI_Agent_Handoff_Charter.md  # this document (Version 5.0)
```
...
torchgenomics impute --genotype data.bed --method mode --output imp.pt
torchgenomics impute --genotype data.bed --method knn --output imp.pt
torchgenomics impute --genotype data.bed --method ld --output imp.pt
...
torchgenomics impute --genotype data.vcf.gz --method impute5 \
  --ref-panel 1000G.vcf.gz --output data_imputed.vcf.gz
torchgenomics impute --genotype data.vcf.gz --method minimac4 \
...
*This charter is the canonical reference for the TorchGenomics project. A new AI agent should treat it
